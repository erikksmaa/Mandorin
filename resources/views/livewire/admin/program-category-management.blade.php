<div>
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
            ...
            <p class="text-slate-500 text-sm">Kelola daftar kategori program kepemudaan yang tersedia di platform SIPORA.</p>
        </div>
        <div class="flex items-center gap-3">
            <div class="w-64 relative">
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari kategori..." class="w-full pl-9 pr-3 py-1.5 text-xs bg-white border border-slate-200 rounded-xl focus:border-navy focus:ring-navy">
            ...
            <table class="w-full text-left text-sm text-slate-600">
                <thead class="bg-slate-50 text-slate-800 border-b border-slate-200 font-semibold">
                    <tr>
                        ...
                        <th class="px-6 py-3.5 text-center">Jumlah Program</th>
                        ...
                        <tr class="hover:bg-slate-50/80 transition">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <button @click="Swal.fire({ icon: 'warning', title: 'Hapus Kategori?', text: 'Apakah Anda yakin ingin menghapus kategori ini?', showCancelButton: true, confirmButtonText: 'Ya, Hapus', cancelButtonText: 'Batal', confirmButtonColor: '#ef4444', cancelButtonColor: '#64748b' }).then(r => { if (r.isConfirmed) $wire.delete({{ $cat->id }}); })" class="p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition" title="Hapus">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-slate-400">
                                Tidak ada data kategori program.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
                    ...
                    <div class="bg-white px-6 pt-6 pb-4">
                        <div class="flex items-center justify-between pb-4 mb-4 border-b border-slate-100">
                            <h3 class="text-lg font-bold text-slate-800" id="modal-title">
                                {{ $editingCategoryId ? 'Edit Kategori Program' : 'Tambah Kategori Program' }}
                            </h3>
                        ...
                        <form wire:submit="save" class="space-y-4">
                            <div>
                                ...
                                <input wire:model="name" id="name" type="text" required placeholder="Contoh: Kepemudaan & Olahraga" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:border-navy outline-none">
