<div>
    <a href="{{ route('leader.programs.show', $program) }}" class="text-sm font-medium text-slate-500 hover:text-slate-800 mb-6 inline-flex items-center gap-1">
        &larr; Kembali ke Detail Program
    </a>

    <h1 class="text-2xl font-bold text-slate-800 mb-6" style="font-family: 'Big Shoulders Display', sans-serif;">
        {{ $worker ? 'Edit Anggota' : 'Tambah Anggota' }}
    </h1>
    ...
    <div class="bg-white border border-slate-200 shadow-sm rounded-2xl p-6 max-w-2xl">
        <form wire:submit="save">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="role" wire:model="role" placeholder="Contoh: Anggota Kayu, Kenek, Ketua Pelaksana" class="w-full rounded-xl border-slate-300 shadow-sm focus:border-orange-500 focus:ring-orange-500" required>
                @error('role') <span class="text-sm text-red-600 mt-1 block">{{ $message }}</span> @enderror
            </div>

            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                <a href="{{ route('leader.programs.show', $program) }}" class="px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 rounded-xl transition border border-transparent">
                    Batal
                </a>
