<div class="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 mb-6">
    ...
    @if($program->members->isEmpty())
        <div class="text-center py-8">
            <p class="text-slate-500">Belum ada anggota di program ini.</p>
        </div>
    @else
        <div class="overflow-x-auto mb-4">
            <table class="w-full text-left text-sm text-slate-600">
                <thead class="bg-slate-50 text-slate-700">
                    <tr>
                        <th class="px-4 py-3 font-medium rounded-l-lg">Nama Anggota</th>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex justify-center gap-4">
                                    <label class="inline-flex items-center gap-1 cursor-pointer">
                                        <input type="radio" wire:model="attendances.{{ $member->id }}" value="present" class="text-navy focus:ring-navy">
                                        <span class="text-xs">Hadir</span>
                                    </label>
                                    <label class="inline-flex items-center gap-1 cursor-pointer">
                                        <input type="radio" wire:model="attendances.{{ $member->id }}" value="absent" class="text-navy focus:ring-navy">
                                        <span class="text-xs">Absen</span>
                                    </label>
                                    <label class="inline-flex items-center gap-1 cursor-pointer">
                                        <input type="radio" wire:model="attendances.{{ $member->id }}" value="leave" class="text-navy focus:ring-navy">
